...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS("🚀 Début du peuplement de la base de données inventory")
        )
        ...
        try:
            with transaction.atomic():
                ...
                self.stdout.write("=" * 60)
                self.stdout.write(
                    self.style.SUCCESS("🎉 Peuplement terminé avec succès!")
                )
    ...
    def create_products_and_stock(self, categories):
        ...
        for product_data in products_data:
            try:
                ...
                self.stdout.write(f"  ✅ Produit créé: {product.name} (Stock: {stock.quantity_available})")
                
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f"  ❌ Erreur lors de la création du produit {product_data['name']}: {e}")
                )